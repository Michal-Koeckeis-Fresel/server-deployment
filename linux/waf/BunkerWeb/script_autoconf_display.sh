...
main() {
    ...
    # Array of files to download (excluding BunkerWeb.conf and syslog-ng.conf - handled separately)
    FILES=(
    ...
    # Handle syslog directory (rsyslog doesn't need custom config file)
    log_step "Setting up syslog directory..."
    mkdir -p "/data/BunkerWeb/syslog"
    mkdir -p "/data/BunkerWeb/logs"
    
    # Note: rsyslog/rsyslog:latest works out of the box without custom config
    log_success "✓ Syslog directories created (rsyslog works without custom config)"
    ...
    # Set appropriate permissions for syslog directories
    chmod 755 "/data/BunkerWeb/syslog"
    chmod 755 "/data/BunkerWeb/logs"
    ...
    echo "📁 Syslog directory: /data/BunkerWeb/syslog/ (rsyslog - no config needed)"
    ...
    echo ""
    echo "Syslog directories:"
    ls -la /data/BunkerWeb/syslog/
    ls -la /data/BunkerWeb/logs/
    echo ""
    ...
    echo ""
    log_info "Next steps:"
    echo "1. Edit /root/BunkerWeb.conf with your domain and email"
    echo "2. Run: ./script_autoconf_display.sh --type autoconf"
    echo ""
    log_info "Note: Using official rsyslog/rsyslog:latest image (no config file needed)"
}
