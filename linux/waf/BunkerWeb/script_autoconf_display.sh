...
REDIS_ENABLED=“no”      # Enable Redis support: yes or no (default: no)
REDIS_PASSWORD=””       # Redis password (auto-generated if Redis enabled)

# Syslog Configuration (optional)

SYSLOG_ENABLED=“no”     # Enable external syslog: yes or no (default: no)
...
# Set REDIS_ENABLED=“yes” to enable Redis support for caching

# REDIS_PASSWORD will be auto-generated for security

# 

# SYSLOG CONFIGURATION:

# Set SYSLOG_ENABLED=“yes” to enable external syslog logging

# Configure SYSLOG_ADDRESS to your syslog server

# Default settings keep syslog local for security
...
echo -e “${GREEN}• REDIS_ENABLED is set to "no" by default${NC}”
echo -e “${GREEN}• Set to "yes" to enable Redis caching support${NC}”
echo “”
echo -e “${BLUE}Syslog Configuration:${NC}”
echo -e “${GREEN}• SYSLOG_ENABLED is set to "no" by default${NC}”
echo -e “${GREEN}• Set to "yes" for external syslog logging${NC}”
echo “”
...
echo -e “  –redis-enabled yes|no       Enable Redis support (default: no)”
echo -e “  –redis-password PASS        Set custom Redis password”
echo “”
echo -e “${YELLOW}Syslog Configuration:${NC}”
echo -e “  –syslog-enabled yes|no      Enable external syslog (default: no)”
...
echo -e “${BLUE}Redis Configuration:${NC}”
echo -e “  ${GREEN}REDIS_ENABLED is set to "no" by default${NC}”
echo -e “  ${GREEN}Enable for caching support and improved performance${NC}”
echo -e “  ${GREEN}Redis password auto-generated with 264-bit entropy${NC}”
echo “”
echo -e “${BLUE}Syslog Configuration:${NC}”
echo -e “  ${GREEN}SYSLOG_ENABLED is set to "no" by default${NC}”
echo -e “  ${GREEN}Enable for centralized logging to external syslog server${NC}”
echo -e “  ${GREEN}Default settings keep logs local for security${NC}”
echo “”
...
echo -e “  sudo $0 –type autoconf –redis-enabled yes”
echo -e “  sudo $0 –type autoconf –syslog-enabled yes –syslog-address syslog.example.com”
...
EXISTING_REDIS_PASSWORD=$(grep "Redis Password:" "$ROOT_CREDS_FILE" | cut -d' ' -f3 || echo "")

# Use existing passwords or generate new ones as needed
...
if [[ "$REDIS_ENABLED" == "yes" ]]; then
    if [[ -n "$EXISTING_REDIS_PASSWORD" ]]; then
        ...
        REDIS_PASSWORD=$(generate_mysql_password)
    fi
fi
...
if [[ -n "$MYSQL_PASSWORD" && -n "$TOTP_SECRET" && -n "$ADMIN_PASSWORD" && -n "$FLASK_SECRET" ]]; then
    ...
    if [[ "$REDIS_ENABLED" == "yes" ]]; then
        if [[ -n "$EXISTING_REDIS_PASSWORD" ]]; then
            echo -e "${GREEN}✓ Existing Redis password loaded${NC}"
        else
    ...
    if [[ "$REDIS_ENABLED" == "yes" && -z "$REDIS_PASSWORD" ]]; then
        REDIS_PASSWORD=$(generate_mysql_password)
        echo -e "${GREEN}✓ New Redis password generated (264-bit)${NC}"
    fi
fi
```
...
# Generate Redis password if enabled - 264-bit
if [[ "$REDIS_ENABLED" == "yes" && -z "$REDIS_PASSWORD" ]]; then
    REDIS_PASSWORD=$(generate_mysql_password)
    echo -e "${GREEN}✓ Redis password generated (264-bit)${NC}"
elif [[ "$REDIS_ENABLED" == "yes" ]]; then
...
if [[ “$REDIS_ENABLED” == “yes” ]]; then
echo -e “${GREEN}• Redis enabled: 264-bit entropy (${#REDIS_PASSWORD} chars)${NC}”
else
echo -e “${BLUE}• Redis disabled${NC}”
fi
if [[ “$SYSLOG_ENABLED” == “yes” ]]; then
echo -e “${GREEN}• Syslog enabled: $SYSLOG_ADDRESS:$SYSLOG_PORT${NC}”
else
echo -e “${BLUE}• Syslog disabled (local logging)${NC}”
fi
...
# SECURITY LEVEL: MAXIMUM+ (264-bit MySQL/Redis passwords)
...
# MySQL/Redis passwords: 264-bit entropy (~44 characters, beyond AES-256)
...
$(if [[ “$REDIS_ENABLED” == “yes” ]]; then
echo “Redis Access: Available with 264-bit password”
fi)
...
echo “Syslog Network: $SYSLOG_NETWORK”
fi)
...
# Redis disabled - use a placeholder or remove Redis configuration
...
if [[ “$REDIS_ENABLED” == “yes” ]]; then
echo -e “${YELLOW}• Redis Access Command:${NC} docker exec -it bw-redis redis-cli -a ‘$REDIS_PASSWORD’”
fi
echo -e “${YELLOW}• Syslog Enabled:${NC} $SYSLOG_ENABLED”
if [[ “$SYSLOG_ENABLED” == “yes” ]]; then
echo -e “${YELLOW}• Syslog Configuration:${NC} $SYSLOG_ADDRESS:$SYSLOG_PORT”
fi
echo “”
...
if [[ “$REDIS_ENABLED” == “yes” ]]; then
echo -e “${GREEN}• Redis caching enabled with 264-bit password${NC}”
fi
if [[ “$SYSLOG_ENABLED” == “yes” ]]; then
echo -e “${GREEN}• External syslog logging enabled: $SYSLOG_ADDRESS:$SYSLOG_PORT${NC}”
else
echo -e “${BLUE}• Syslog disabled - using local logging only${NC}”
fi
echo “”
...
if [[ $SETUP_MODE == "automated" ]]; then
    ...
    if [[ "$REDIS_ENABLED" == "yes" ]]; then
        echo -e "${BLUE}🔴 Redis caching (264-bit): docker exec -it bw-redis redis-cli -a '$REDIS_PASSWORD'${NC}"
    fi
    if [[ -n "$AUTO_CERT_TYPE" ]]; then
