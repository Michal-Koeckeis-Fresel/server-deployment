...
if [[ "$SYSLOG_ENABLED" == "yes" ]]; then
  ...
  bw-syslog:
    ...
    ports:
      ...
      - "514:514/udp"
    environment:
      - RSYSLOG_CONF=*.info /var/log/messages;*.warn /var/log/syslog;*.err /var/log/error.log;*.* /dev/stdout
    volumes:
...
# Rest of the script continues with SSL configuration, multisite setup, etc...
# [Previous SSL, multisite, and other configuration code remains the same]
