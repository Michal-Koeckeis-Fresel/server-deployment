...
is_bunkerweb_conf_valid() {
    ...
    # If file doesn't exist, it's invalid
    if [ ! -f "$file" ]; then
        return 1
    fi
    ...
    # If file is 0 or 1 byte, it's invalid
    if [ "$file_size" -le 1 ]; then
        return 1
    fi
    
    # Check if file contains only whitespace/comments
...
main "$@" "$file" 2>/dev/null | \
    ...
    # If no meaningful content, it's invalid
